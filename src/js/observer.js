...
(function () {
  ...
  function initObserver() {
    setTimeout(() => {
      ...
      // Re-trigger scroll animations inside preview canvas on hover
      if (!document.sdsScrollDelegationBound) {
        document.sdsScrollDelegationBound = true;

        document.addEventListener('mouseover', e => {
          const canvas = e.target.closest('.preview-canvas');
          if (!canvas) return;
          
          if (canvas.dataset.sdsHovered === "true") return;
          canvas.dataset.sdsHovered = "true";

          const scrollEls = canvas.querySelectorAll('[class*="sds-scroll-"]');
          scrollEls.forEach(scrollEl => {
            scrollEl.classList.remove('sds-in-view');
            void scrollEl.offsetWidth; // force layout reflow
            scrollEl.classList.add('sds-in-view');
          });
        }, true);
        ...
        document.addEventListener('mouseout', e => {
          ...
          if (!related || !canvas.contains(related)) {
            canvas.dataset.sdsHovered = "false";
          }
        }, true);
      }
    }, 50);
